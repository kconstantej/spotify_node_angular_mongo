...
function getSongs(req, res){
    ...
    find.populate({
        path: 'album',
        //va a popular el dato de artista para que salga todo los datos de artista 
        populate: {
            path: 'artist',
            model: 'Artist'
        }
...
function deleteSong(req, res){
    var sonId = req.params.id;
    Song.findByIdAndRemove(sonId,(err,songRemove)=>{
        if(err){
            res.status(500).send({message: 'Error en el servidor'});
        }else{
            if(!songRemove){
                res.status(404).send({message: 'No se a podido eliminar la cancion'});
            }else{
    ...
    if(req.files){
        ...
        if(file_ext == 'mp3'  ||  file_ext == 'ogg' ){
            Song.findByIdAndUpdate(songId,{file:file_name},(err,songUpdate)=>{
                if(!songUpdate){
                    res.status(404).send({message: 'No se ha pododo actualizar la cancion'}); 
        ...
        res.status(200).send({message: 'No se ha subido la cancion...'}); 
    }
};
