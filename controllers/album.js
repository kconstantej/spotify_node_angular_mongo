...
function getAlbum(req,res){
    ...
    //este es un ejemplo para buscar el album y por medio de populate se busca  el path que seria artist para
    //buscra todo sobre artist
...
function updateAlbum(req, res){
    ...
    Album.findByIdAndUpdate(albumId,update,(err,albumUpdate)=>{
        if(err){
            ...
            if(!albumUpdate){
                res.status(404).send({message: 'No se encontro album'});
    ...
    if(req.files){
        ...
        if(file_ext == 'png'  ||  file_ext == 'jpg'  || file_ext == 'gif'){
            Album.findByIdAndUpdate(albumId,{image:file_name},(err,albumUpdated)=>{
                if(!albumUpdated){
                    res.status(404).send({message: 'No se ha pododo actualizar el album'}); 
